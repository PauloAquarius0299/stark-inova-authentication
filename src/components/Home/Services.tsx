    ...
    <section className='p-8 pb-20 md:pt-5 md:pb-10 bg-[radial-gradient(ellipse_200%_100%_at_top_left,#5a79e9,#eaeefe_100%)] py-20'>
      ...
      <div className='w-full mt-12 grid lg:grid-cols-4 grid-cols-1 gap-10'>
        {workExperience.map((card) => (
          ...
          >
            <div>
              <Image src={card.thumbnail} alt='logo image service' className='lg:w-32 md:w-20 w-16 ml-2' />
              <div className='lg:ms-5'>
                <h1 className='text-start text-xl md:text-2xl font-bold'>{card.title}</h1>
                <p className='text-start text-blue-500 mt-3 font-semibold'>{card.desc}</p>
              </div>
            </div>
          </Button>
        ))}
